...
class TimeSheet(models.Model):
    ...
    paid_on = models.DateField(verbose_name='Paid Date', auto_now=False, blank=True, null=True)
...
class TimeSheetItem(models.Model):
    timesheet = models.ForeignKey(TimeSheet, on_delete=models.CASCADE, verbose_name='Time Sheet ID')
    job_item = models.OneToOneField(JobUser, on_delete=models.SET_NULL, verbose_name="Staff's Job ID", blank=True, null=True)

    total_hours = models.DurationField(verbose_name='Total Hours', blank=True, null=True)
    approved_by = models.CharField(max_length=50, verbose_name='Approved By', blank=True, null=True)
    ...
    def __str__(self):
        return f'{self.total_hours} approved by{self.approved_by.name}'